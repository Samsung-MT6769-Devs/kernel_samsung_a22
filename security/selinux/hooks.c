...
static inline unsigned int cmp_sec_integrity(const struct cred* cred, struct mm_struct* mm)
{
    ...
    return ((cred->bp_task != current) || (mm && (!(in_interrupt() || in_softirq())) && (cred->bp_pgd != swapper_pg_dir) && (mm->pgd != cred->bp_pgd)));
}
...
static inline unsigned int rkp_is_valid_cred_sp(u64 cred, u64 sp)
{
    ...
    if ((cred == (u64)&init_cred) && (sp == (u64)&init_sec))
        return 0;

    if (!rkp_ro_page(cred) || !rkp_ro_page(cred + sizeof(struct cred)) || (!rkp_ro_page(sp) || !rkp_ro_page(sp + sizeof(struct task_security_struct)))) {
...
{
    rcu_read_lock();
    if (rkp_cred_enable && (rkp_is_valid_cred_sp((u64)current_cred(), (u64)current_cred()->security) || cmp_sec_integrity(current_cred(), current->mm) || cmp_ns_integrity())) {
        rcu_read_unlock();
...
static int selinux_is_genfs_special_handling(struct super_block* sb)
{
    ...
    return !strcmp(sb->s_type->name, "sysfs") || !strcmp(sb->s_type->name, "pstore") || !strcmp(sb->s_type->name, "debugfs") || !strcmp(sb->s_type->name, "tracefs") || !strcmp(sb->s_type->name, "rootfs") || (selinux_policycap_cgroupseclabel() && (!strcmp(sb->s_type->name, "cgroup") || !strcmp(sb->s_type->name, "cgroup2")));
}
...
next_inode:
    if (!list_empty(&sbsec->isec_head)) {
        struct inode_security_struct* isec = list_entry(sbsec->isec_head.next,
    ...
    unsigned long* set_kern_flags)
{
    ...
    if (!strcmp(sb->s_type->name, "debugfs") || !strcmp(sb->s_type->name, "tracefs") || !strcmp(sb->s_type->name, "sysfs") ||
        // [ SEC_SELINUX_PORTING_COMMON
        !strcmp(sb->s_type->name, "configfs") ||
        // ] SEC_SELINUX_PORTING_COMMON
        !strcmp(sb->s_type->name, "pstore") || !strcmp(sb->s_type->name, "bpf") || !strcmp(sb->s_type->name, "binder") || !strcmp(sb->s_type->name, "cgroup") || !strcmp(sb->s_type->name, "cgroup2"))
     ...
     */
    if (sb->s_user_ns != &init_user_ns && strcmp(sb->s_type->name, "tmpfs") && strcmp(sb->s_type->name, "ramfs") && strcmp(sb->s_type->name, "devpts")) {
    ...
    unsigned long* set_kern_flags)
{
    ...
    if (newsbsec->behavior == SECURITY_FS_USE_NATIVE && !(kern_flags & SECURITY_LSM_NATIVE_LABELS) && !set_context) {
...
static inline u16 socket_type_to_security_class(int family, int type, int protocol)
{
    ...
    case PF_INET6:
        ...
        case SOCK_DGRAM:
            ...
            else if (extsockclass && (protocol == IPPROTO_ICMP || protocol == IPPROTO_ICMPV6))
...
static int selinux_inode_setattr(struct dentry* dentry, struct iattr* iattr)
{
    ...
    if (selinux_policycap_openperm() && inode->i_sb->s_magic != SOCKFS_MAGIC && (ia_valid & ATTR_SIZE) && !(ia_valid & ATTR_FILE))
        av |= FILE__OPEN;
...
static int selinux_file_permission(struct file* file, int mask)
{
    ...
    isec = inode_security(inode);
    if (sid == fsec->sid && fsec->isid == isec->sid && fsec->pseqno == avc_policy_seqno(&selinux_state))
...
static int selinux_nlmsg_perm(struct sock* sk, struct sk_buff* skb)
{
    ...
    while (data_len >= nlmsg_total_size(0)) {
        ...
        if (rc == 0) {
            ...
            if (enforcing_enabled(&selinux_state) && !security_get_allow_unknown(&selinux_state))
                return rc;
            rc = 0;
        } else if (rc == -ENOENT) {
