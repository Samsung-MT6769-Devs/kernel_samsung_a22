    ...
    struct input_dev* dev)
{
    ...
    for (id = handler->id_table; id->flags || id->driver_info; id++) {
        if (input_match_device_id(dev, id) && (!handler->match || handler->match(handler, dev))) {
            return id;
        }
    }
...
static unsigned int input_estimate_events_per_packet(struct input_dev* dev)
{
    ...
    if (dev->mt) {
        ...
        mt_slots = dev->absinfo[ABS_MT_TRACKING_ID].maximum - dev->absinfo[ABS_MT_TRACKING_ID].minimum + 1,
