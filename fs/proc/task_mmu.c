    ...
    unsigned long* resident, unsigned long* swapresident)
{
    ...
    *size = mm->total_vm;
    *resident = get_mm_counter(mm, MM_FILEPAGES) + get_mm_counter(mm, MM_SHMEMPAGES) + get_mm_counter(mm, MM_ANONPAGES);
    ...
    struct mem_size_stats* mss)
{
    ...
    if (vma->vm_file && shmem_mapping(vma->vm_file->f_mapping)) {
        ...
        if (!shmem_swapped || (vma->vm_flags & VM_SHARED) || !(vma->vm_flags & VM_WRITE)) {
    ...
    if (ptl) {
        ...
        if (pmd_present(pmd)) {
            ...
            if (pmd_soft_dirty(pmd))
                flags |= PM_SOFT_DIRTY;
            if (pm->show_pfn)
                frame = pmd_pfn(pmd) + ((addr & ~PMD_MASK) >> PAGE_SHIFT);
        }
            ...
            if (pm->show_pfn) {
                offset = swp_offset(entry) + ((addr & ~PMD_MASK) >> PAGE_SHIFT);
...
static int show_numa_map(struct seq_file* m, void* v)
{
    ...
    for_each_node_state(nid, N_MEMORY) if (md->node[nid])
...
show_filemap_vma(struct seq_file* m, struct vm_area_struct* vma)
{
    ...
    if (priv->show_list) {
        if (!strncmp(pathname, "/data", 5) || !strncmp(pathname, "/system", 7)) {
            seq_puts(m, pathname);
            seq_putc(m, '\n');
        }
    }
