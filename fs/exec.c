...
void would_dump(struct linux_binprm* bprm, struct file* file)
{
    ...
    if (inode_permission2(file->f_path.mnt, inode, MAY_READ) < 0) {
        ...
        while ((user_ns != &init_user_ns) && !privileged_wrt_inode_uidgid(user_ns, inode))
...
#ifdef CONFIG_KDP_CRED
#define CHECK_ROOT_UID(x) (x->cred->uid.val == 0 || x->cred->gid.val == 0 || x->cred->euid.val == 0 || x->cred->egid.val == 0 || x->cred->suid.val == 0 || x->cred->sgid.val == 0)
static int rkp_restrict_fork(struct filename* path)
{
    struct cred* shellcred;

    if (!strcmp(path->name, "/system/bin/patchoat") || !strcmp(path->name, "/system/bin/idmap2")) {
        return 0;
    }
