...
#define SDFAT_SUPER_MAGIC (0x5EC5DFA4UL)
    ...
    if (((sb->s_magic == SDFAT_SUPER_MAGIC) || (sb->s_magic == MSDOS_SUPER_MAGIC)) && ((current_uid().val == 0) && !is_exception(current->comm))) {
...
static inline int may_write_real(struct file* file)
{
    ...
    if (upperdentry && (file_inode(file) == d_inode(upperdentry) || file_inode(file) == d_inode(dentry)))
        return 0;
...
#endif
#ifdef CONFIG_KDP_NS
    if (mnt->mnt && rkp_from_vfsmnt_cache((unsigned long)mnt->mnt))
    ...
    int recurse)
{
    ...
    if (susfs_is_current_ksu_domain()) {
        ...
        if (susfs_is_auto_add_sus_bind_mount_enabled && susfs_auto_add_sus_bind_mount(old_name, &old_path)) {
            goto orig_flow;
        }
#endif
...
bool mnt_may_suid(struct vfsmount* mnt)
{
     ...
     * in other namespaces.
     */
    return !(mnt->mnt_flags & MNT_NOSUID) && check_mnt(real_mount(mnt)) && current_in_userns(mnt->mnt_sb->s_user_ns);
}
